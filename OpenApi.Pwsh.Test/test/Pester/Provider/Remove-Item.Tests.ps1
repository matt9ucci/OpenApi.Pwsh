...
Describe '<RemoveItem>' -ForEach @(
	@{ RemoveItem = { Remove-Item -Path $Path -Recurse } }
	@{ RemoveItem = { Remove-Item -LiteralPath $Path -Recurse } }
	@{ RemoveItem = { $Path | Remove-Item -Recurse } }
	@{ RemoveItem = { [pscustomobject]@{ Path = $Path } | Remove-Item -Recurse } }
) {
	BeforeAll {
...
Describe 'Remove-Item -Path wildcard' {
	BeforeAll {
		New-PSDrive OpenApi ([OpenApiProvider]::ProviderName) Root
	}
...
Describe 'Remove-Item exceptions' {
	BeforeAll {
		New-PSDrive OpenApi ([OpenApiProvider]::ProviderName) Root
	}
	...
	It 'cannot remove <Path>' -TestCases @(
		@{ Segments = @('OpenApi:') }
	) {
		{
